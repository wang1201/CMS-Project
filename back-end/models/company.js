...
//自动起名为负数
var company = mongoose.model('company', companyCollection);

// 查询数据列表-全部
const listAll = () => {
    let _query = {}// 查询的约定条件，因为考虑到搜索的时候也是查询，但是那时候是有条件的
    //mongoose的查询-表.find({条件}).sort({createTime:-1})升序   1 降序 
    return company.find(_query).sort({createTime: -1}).then((results) => {
        return results
    }).catch((err) => {
        return false
    })
...
const listSome = async ({ pageNo = 1, pageSize = 10, search = '' }) => {
    ...
    return company.find(_query)//moongoose语法，数据库中查找符合条件的
...
// 查询数据列表-根据id查询某一条
const listOne = ({ id }) => { 
    return company.findById(id).then((results) => {
        return results
    }).catch((err) => {
        return false
    })
...
const update = (body) => {
    ...
    return company.updateOne({ _id: body.id }, { ...body }).then((results) => {
        return results
    }).catch((err) => {
        return false
    }) 
...
const remove = async ({ id, pageNo, pageSize }) => {
    ...
    return company.deleteOne({ _id: id }).then(async (results) => {
        //  获取最新的数量
