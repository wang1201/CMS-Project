...
const listAll = async (req, res) => {
    let _data = await company_model.listAll()
...
const listOne = async (req, res) => {
    let _data = await company_model.listOne(req.query)
    handleData(_data, res, 'company')
}
...
const update = async (req, res, next) => {
    ...
    res.set('content-type', 'application/json; charset=utf8');
    let _data = await company_model.update(req.body);
    handleData(_data, res, 'company')
}
...
const remove = async (req, res, next) => {
    ...
    let _data = await company_model.remove(req.query);
    handleData(_data, res, 'company')
}
module.exports = {
    listAll,
